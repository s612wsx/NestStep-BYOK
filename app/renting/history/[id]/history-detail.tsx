...
const backLink = (
  ...
  >
    <span aria-hidden>←</span> 分析紀錄列表
  </Link>
);

export function HistoryDetail({ id }: { id: string }) {
  const [state, setState] = useState<State>("loading");
  const [data, setData] = useState<RentingHistoryDetail | null>(null);

  useEffect(() => {
    let cancelled = false;
    fetch(`/api/renting/history/${id}`)
      .then(async (res) => {
        if (res.status === 401) {
        ...
        if (!cancelled) {
          setData(json as RentingHistoryDetail);
          setState("ready");
        }
      })
      .catch(() => {
    ...
    <div className="space-y-7">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-stone-500">
          ...
          {INPUT_TYPE_LABEL[data.inputType] ?? data.inputType}
        </p>
      </div>
      ...
      <section>
        ...
        <p className="mt-2.5 whitespace-pre-wrap text-[15px] leading-relaxed text-stone-700 dark:text-stone-300">
          {data.originalText || "（沒有文字內容）"}
        </p>
      </section>

      <div>
        <h3 className="mb-3 text-sm font-semibold">AI 分析結果</h3>
        <AnalysisSections result={data.analysisResult} />
      </div>
    </div>
  );
}
