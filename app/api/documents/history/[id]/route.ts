...
export async function GET(
  ...
  try {
    await connectMongo();
    const doc = await DocumentAnalysisModel.findById(id).lean();
    ...
    console.error("[documents/history/:id GET]", err);
  ...
  try {
    ...
    if (doc.fileUrl) {
      try {
        ...
        console.error(
          "[documents/history/:id DELETE] Blob 刪除失敗，未刪除 DB 紀錄：",
          err,
        );
        return NextResponse.json(
    ...
    await DocumentAnalysisModel.deleteOne({ _id: id, userId: session.id });

    return NextResponse.json({ ok: true, id });
  } catch (err) {
    console.error("[documents/history/:id DELETE]", err);
