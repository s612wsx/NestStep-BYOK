...
import type { LifeQueryHistoryItem } from "@/app/lib/life-query-types";

type State = "loading" | "ready" | "unauth" | "error";

export function OtherHistoryList() {
  const [state, setState] = useState<State>("loading");
  const [items, setItems] = useState<LifeQueryHistoryItem[]>([]);
  ...
  const handleDelete = async (id: string) => {
    if (
      !window.confirm("確定要刪除這筆紀錄嗎？連同上傳的照片一起刪除，無法復原。")
    ) {
      return;
    }
    setDeletingId(id);
    setRowError(null);
    try {
      const res = await fetch(`/api/other/history/${id}`, { method: "DELETE" });
              ...
              >
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-0.5 block text-sm text-stone-500">
                    ...
                    {item.detectedCategory ? ` · ${item.detectedCategory}` : ""}
                    {item.hasImage ? " · 📎" : ""}
                  </span>
                </span>
                <span
